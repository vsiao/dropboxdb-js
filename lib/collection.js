...
Collection.prototype._syncOptions = function(fetch) {
  ...
  ths._client.stat(ths._path, function(error, stat) {
    if ((error && error.status === 404) || (!error && stat.isRemoved)) {
      ...
      ths._client.writeFile(ths._dbdbfile, JSON.stringify(ths._options), noop);
    } else if (error) {
      console.log(error);
    } else {
      if (fetch) {
...
Collection.prototype.insert = function(attributes, callback) {
  ...
  var ths = this;
  this._client.readFile(this._path + key, function(error, str, stat) {
    if (error) {
      if (error.status != 404) {
        ...
        console.log("INSERTION ERROR: ELEMENT ALREADY EXISTS. USE UPDATE");
        return;
      }
...
Collection.prototype.find = function(query, callback) {
  ...
  ths._client.readdir(ths._path, function(error, entries) {
    ...
    var rows = new Array(entries.length - 1);
    ...
    entries.forEach(function(entry) { 
      if(entry !== "__DBDB__") {
        console.log("trying reading file: " + entry);
