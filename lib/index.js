...
DropboxDB.prototype.insert = function(collection, row, cb) {
  function ondone() {
    this.collections[collection].insert(row, cb);
  }
  if (!this.collections.hasOwnProperty(collection)) {
    this.create(collection, {}, ondone);
  } else {
    ondone();
  }
}
...
DropboxDB.prototype.find = function(collection, query, fields, cb) {
  client = this._client
  client.readdir("/dropboxdb/" + collection, function(error, entries) {
    if (error) {
      return;
    }
    ...
    entries.each(function(entry) { 
      client.readFile("/dropboxdb/" + entry, sync) 
    });
