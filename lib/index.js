...
DropboxDB.prototype.insert = function(collection, row, cb) {
  ...
  if (!ths.collections.hasOwnProperty(collection)) {
    console.log('/dropboxdb/' + collection + '/___DBDB___');
    ths._client.readFile('/dropboxdb/' + collection + '/___DBDB___', function(error, str, stat) {
      if (error) { console.log(collection); return; }
      ths.collections[collection] = new Collection(ths._client, collection, JSON.parse(str));
      ondone();
      });
  } else {
    ondone();
...
DropboxDB.prototype.remove = function(collection, primaryKey) {
  this._client.remove("/dropboxdb/" + collection + "/" + primaryKey, function(error, stat) {
    if (error) {
      return;
    }
  });
...
DropboxDB.prototype.find = function(collection, query, cb) {
  ...
  if (!ths.collections.hasOwnProperty(collection)) {
    console.log("need to create collection");
